...
contract('TrashReward', async accounts => {
    ...
    it('Can add container as contract owner', async () => {
      try {
        ...
        const logs = result.logs.filter(
          entry => entry.event === 'ContainerAdded'
        )
  ...
  describe('#removeContainer()', () => {
    ...
    it('Can add container as contract owner', async () => {
      try {
        const result = await contractInstance.removeContainer(ACC_CONTAINER, {
          from: ACC_OWNER
        })
  ...
  describe('#rewardCitizen()', () => {
    ...
    it('Can not reward citizen without enough founds', async () => {
      try {
        const result = await contractInstance.rewardCitizen(
          ACC_CITIZEN,
          REWARD_AMOUNT + 1,
          {
            from: ACC_CONTAINER
        ...
        const logs = result.logs.filter(
          entry => entry.event === 'InsufficiantFounds'
        )
        ...
        assert(
          false,
          'Reward citizen without enough found should not fail but report!'
        )
      }
    })
